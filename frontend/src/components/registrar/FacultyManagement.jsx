...
const FacultyManagement = () => {
  ...
  const fetchFaculty = async () => {
    try {
      ...
      const response = await axios.get('https://82.112.238.4:5000/api/registrar/faculty', {
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = editingFaculty 
        ? `https://82.112.238.4:5000/api/registrar/faculty/${editingFaculty.id}`
        : 'https://82.112.238.4:5000/api/registrar/faculty';
  ...
  const handleDelete = async (id, name) => {
    ...
    try {
      const token = localStorage.getItem('token') || sessionStorage.getItem('token');
      const response = await axios.delete(`https://82.112.238.4:5000/api/registrar/faculty/${id}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });

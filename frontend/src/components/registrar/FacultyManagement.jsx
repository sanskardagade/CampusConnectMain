...
const FacultyManagement = () => {
  ...
  const fetchFaculty = async () => {
    try {
      ...
      const response = await axios.get('http://82.112.238.4:5000/api/registrar/faculty', {
        headers: {
  ...
  const fetchDepartments = async () => {
    try {
      const token = localStorage.getItem('token') || sessionStorage.getItem('token');
      const response = await axios.get('http://82.112.238.4:5000/api/registrar/departments', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = editingFaculty 
        ? `http://82.112.238.4:5000/api/registrar/faculty/${editingFaculty.id}`
        : 'http://82.112.238.4:5000/api/registrar/faculty';
  ...
  const handleDelete = async (id, name) => {
    ...
    try {
      const token = localStorage.getItem('token') || sessionStorage.getItem('token');
      const response = await axios.delete(`http://82.112.238.4:5000/api/registrar/faculty/${id}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });

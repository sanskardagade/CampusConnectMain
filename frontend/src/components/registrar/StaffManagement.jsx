...
const StaffManagement = () => {
  ...
  const fetchStaff = async () => {
    try {
      ...
      const response = await axios.get('http://69.62.83.14:5000/api/registrar/staff', {
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = editingStaff 
        ? `http://69.62.83.14:5000/api/registrar/staff/${editingStaff.id}`
        : 'http://69.62.83.14:5000/api/registrar/staff';
  ...
  const handleDelete = async (id, name) => {
    ...
    try {
      const token = localStorage.getItem('token') || sessionStorage.getItem('token');
      const response = await axios.delete(`http://69.62.83.14:5000/api/registrar/staff/${id}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });

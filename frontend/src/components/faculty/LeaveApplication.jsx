...
export default function LeaveApplication() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        const response = await fetch(`http://82.112.238.4:5000/api/faculty/dashboard?date=${today}`, {
          headers: {
  ...
  useEffect(() => {
    const fetchLeaveApplications = async () => {
      try {
        const response = await fetch('http://82.112.238.4:5000/api/faculty/leave-apply', {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
  ...
  useEffect(() => {
    const fetchLeaveBalances = async () => {
      try {
        const response = await fetch('http://82.112.238.4:5000/api/faculty/leave-balances', {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const applicationsResponse = await fetch('http://82.112.238.4:5000/api/faculty/leave-apply', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });

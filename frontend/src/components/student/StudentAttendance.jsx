...
const AttendanceDashboard = () => {
  ...
  const fetchAttendanceSummary = async () => {
    try {
      setLoading(true);
      const response = await api.get('http://82.112.238.4:5000/api/studentAttendance/summary');
  ...
  const fetchAttendanceDetails = async () => {
    try {
      ...
      const response = await api.get('http://82.112.238.4:5000/api/details', { params });

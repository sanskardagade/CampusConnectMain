...
export default function AdminLeaveApplication() {
  ...
  useEffect(() => {
    const fetchLeaveApplications = async () => {
      try {
        const response = await axios.get('http://69.62.83.14:5000/api/admin/faculty-leave-approval', {
          headers: {
  ...
  const handleApprove = async (application) => {
    ...
    try {
      const response = await axios.put(
        `http://69.62.83.14:5000/api/admin/faculty-leave-approval/${application.ErpStaffId}`,
  ...
  const handleReject = async () => {
    ...
    try {
      const response = await axios.put(
        `http://69.62.83.14:5000/api/admin/faculty-leave-approval/${selectedApplication.ErpStaffId}`,

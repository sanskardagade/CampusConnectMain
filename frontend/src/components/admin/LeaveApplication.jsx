...
export default function AdminLeaveApplication() {
  ...
  useEffect(() => {
    const fetchLeaveApplications = async () => {
      try {
        const response = await axios.get('http://82.112.238.4:5000/api/admin/faculty-leave-approval', {
          headers: {
  ...
  const handleApprove = async (application) => {
    ...
    try {
      const response = await axios.put(
        `http://82.112.238.4:5000/api/admin/faculty-leave-approval/${application.ErpStaffId}`,
  ...
  const handleReject = async () => {
    ...
    try {
      const response = await axios.put(
        `http://82.112.238.4:5000/api/admin/faculty-leave-approval/${selectedApplication.ErpStaffId}`,

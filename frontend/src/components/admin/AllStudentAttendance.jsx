...
const AdminAllStudentAttendance = () => {
  ...
  const fetchAttendanceData = async () => {
    try {
      ...
      const response = await fetch(`http://69.62.83.14:5000/api/admin/all-student-attendance?${new URLSearchParams(filters)}`, {
        headers: {
  ...
  const downloadReport = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://69.62.83.14:5000/api/attendance/download/all?${new URLSearchParams(filters)}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });

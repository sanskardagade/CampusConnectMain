...
const AdminAllStudentAttendance = () => {
  ...
  const fetchAttendanceData = async () => {
    try {
      ...
      const response = await fetch(`https://api.antryx.in/api/admin/all-student-attendance?${new URLSearchParams(filters)}`, {
        headers: {
  ...
  const downloadReport = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://api.antryx.in/api/attendance/download/all?${new URLSearchParams(filters)}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });

...
import ShortLeaveApproval from './ShortLeaveApproval';
...
export default function LeaveApplication() {
  ...
  const [selectedView, setSelectedView] = useState('Regular'); // Regular | Short
  const [hodPendingShortCount, setHodPendingShortCount] = useState(0);
  ...
  // Fetch short leave HOD-pending count
  useEffect(() => {
    const fetchShortLeavesHodPending = async () => {
      try {
        const res = await axios.get('https://82.112.238.4:5000/api/principal/short-leaves', {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        const count = (res.data || []).filter(item => item.HodStatus === 'Pending').length;
        setHodPendingShortCount(count);
      } catch (e) {
        setHodPendingShortCount(0);
      }
    };
    fetchShortLeavesHodPending();
  }, [token]);

  useEffect(() => {
    const fetchLeaveApplications = async () => {
      try {
        const response = await axios.get('https://82.112.238.4:5000/api/principal/faculty-leave-approval', {
          headers: {
  ...
  const handleApprove = async (application) => {
    ...
    try {
      const response = await axios.put(
        `https://82.112.238.4:5000/api/principal/faculty-leave-approval/${application.ErpStaffId}`,
  ...
  const handleReject = async () => {
    ...
    try {
      const response = await axios.put(
        `https://82.112.238.4:5000/api/principal/faculty-leave-approval/${selectedApplication.ErpStaffId}`,
            ...
            <div className="flex mb-4 rounded-lg overflow-hidden shadow divide-x divide-gray-200 bg-white">
              {['Regular', 'Short'].map(view => (
                <button
                  ...
                  className={`relative flex-1 px-4 py-2 font-semibold text-sm transition-colors duration-150 ${
                    selectedView === view
                      ? (view === 'Regular' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800')
                      : 'bg-white text-gray-600 hover:bg-gray-100'
                  }`}
                >
                  {view === 'Regular' ? 'Leaves' : 'Short Leaves'}
                  {view === 'Short' && hodPendingShortCount > 0 && (
                    <span className="absolute -top-2 -right-2 text-xs font-semibold px-2 py-0.5 rounded-full bg-yellow-100 text-yellow-800 border border-yellow-200" title="HOD Pending Short Leaves">
                      {hodPendingShortCount}
                    </span>
                  )}
                </button>
              ))}

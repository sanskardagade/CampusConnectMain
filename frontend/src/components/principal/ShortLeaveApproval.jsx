...
export default function ShortLeaveApproval() {
  ...
  const handleApprove = async (app) => {
    try {
      ...
      if (!token) throw new Error('Missing auth token');
      await axios.put(
        `https://82.112.238.4:5000/api/principal/short-leaves/${app.id}`,
  ...
  const hodPendingCount = applications.filter(a => a.HodStatus === 'Pending').length;
        ...
        <div className="bg-white rounded-lg shadow-md p-4">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              {hodPendingCount > 0 && (
                <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-yellow-100 text-yellow-800" title="Pending at HOD">
                  HOD Pending: {hodPendingCount}
                </span>
              )}
            </div>

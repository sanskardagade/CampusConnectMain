...
const FacultyDashboard = () => {
  ...
  const fetchTasks = async () => {
    try {
      const response = await axios.get('http://69.62.83.14:5000/api/faculty/assigned-tasks', {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        const response = await axios.get('http://69.62.83.14:5000/api/faculty/students-data', {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
        });
  ...
  const fetchTaskHistory = async () => {
    try {
      const response = await axios.get('http://69.62.83.14:5000/api/faculty/task-history', {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  const handleAcceptTask = async (id) => {
    ...
    try {
      await axios.patch(`http://69.62.83.14:5000/api/faculty/assigned-tasks/${id}/accept`, {}, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
      await fetchTasks();
    } finally {
  ...
  const handleRejectTask = async (id) => {
    setSubmittingTaskId(id);
    try {
      await axios.patch(`http://69.62.83.14:5000/api/faculty/assigned-tasks/${id}/reject`, {}, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
      await fetchTasks();
    } finally {
  ...
  const handleCompleteTask = async (id) => {
    ...
    try {
      ...
      setUploading(false);
      await axios.patch(`http://69.62.83.14:5000/api/faculty/assigned-tasks/${id}/complete`, {
        responseFileUrl,
        submissionMessage
      }, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
  ...
  const handleDismissTask = async (assignedTaskId) => {
    ...
    try {
      await axios.patch(`http://69.62.83.14:5000/api/faculty/assigned-tasks/${assignedTaskId}/dismiss`, {}, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });

...
const HODDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const facultyResponse = await fetch('https://api.antryx.in/api/hod/faculty', {
          method: 'GET',
          headers: {
        ...
        const logsResponse = await fetch('https://api.antryx.in/api/hod/faculty-log', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
        ...
        const dashboardResponse = await fetch('https://api.antryx.in/api/hod/dashboard', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
        ...
        let staffList = [];
        try {
          const staffRes = await fetch('https://api.antryx.in/api/hod/nonteaching', {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${token}`
        ...
        try {
          const logsRes = await fetch('https://api.antryx.in/api/hod/faculty-log', {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${token}`
  ...
  useEffect(() => {
    const fetchFacultyStress = async () => {
      ...
      try {
        const response = await fetch('https://api.antryx.in/api/faculty/student-stress-level', {
          headers: {
  ...
  const fetchAssignedTasksHistory = useCallback(async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get("https://api.antryx.in/api/hod/assigned-tasks/history", {
  ...
  const handlePersonClick = (type, id, name) => {
    ...
    const url = type === 'faculty'
      ? `https://api.antryx.in/api/hod/faculty-profile/${id}`
      : `https://api.antryx.in/api/hod/staff-profile/${id}`;
    fetch(url, {
    ...
    const fetchTodayCounts = async () => {
      try {
        ...
        const [facultyRes, studentRes, staffRes] = await Promise.all([
          axios.get('https://api.antryx.in/api/hod/faculty-today-attendance-count', { 
            headers: { Authorization: `Bearer ${token}` } 
          }).catch(e => ({ data: { count: 0 } })), // Fallback if error
          axios.get('https://api.antryx.in/api/hod/student-today-attendance-count', { 
            headers: { Authorization: `Bearer ${token}` } 
          }).catch(e => ({ data: { count: 0 } })),
          axios.get('https://api.antryx.in/api/hod/nonteaching-today-attendance-count', { 
            headers: { Authorization: `Bearer ${token}` } 
          }).catch(e => ({ data: { count: 0 } }))
        ]);
...
function RecentFacultyLogs({ facultyMembers = [], handlePersonClick }) {
  ...
  useEffect(() => {
    ...
    const fetchLogs = async () => {
      ...
      try {
        const token = localStorage.getItem('token');
        const res = await fetch('https://api.antryx.in/api/hod/recent-faculty-logs', {
          headers: { 'Authorization': `Bearer ${token}` },
        });
...
function RecentLeaveApprovals() {
  ...
  useEffect(() => {
    const fetchLeaves = async () => {
      try {
        setLoading(true);
        const res = await axios.get('https://api.antryx.in/api/hod/leave-approval');
  ...
  const handleAction = async (application, action) => {
    ...
    try {
      await axios.put(`https://api.antryx.in/api/hod/leave-approval/${application.ErpStaffId}`, {

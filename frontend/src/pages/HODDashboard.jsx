...
const HODDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const facultyResponse = await fetch('http://69.62.83.14:5000/api/hod/faculty', {
          method: 'GET',
          headers: {
        ...
        const logsResponse = await fetch('http://69.62.83.14:5000/api/hod/faculty-log', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
        ...
        const dashboardResponse = await fetch('http://69.62.83.14:5000/api/hod/dashboard', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
        ...
        let staffList = [];
        try {
          const staffRes = await fetch('http://69.62.83.14:5000/api/hod/nonteaching', {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${token}`
        ...
        try {
          const logsRes = await fetch('http://69.62.83.14:5000/api/hod/faculty-log', {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${token}`
  ...
  const fetchAssignedTasksHistory = useCallback(async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get("http://69.62.83.14:5000/api/hod/assigned-tasks/history", {
  ...
  const handlePersonClick = (type, id, name) => {
    ...
    const url = type === 'faculty'
      ? `http://69.62.83.14:5000/api/hod/faculty-profile/${id}`
      : `http://69.62.83.14:5000/api/hod/staff-profile/${id}`;
    fetch(url, {
    ...
    const fetchTodayCounts = async () => {
      try {
        ...
        const [facultyRes, studentRes, staffRes] = await Promise.all([
          axios.get('http://69.62.83.14:5000/api/hod/faculty-today-attendance-count', { 
            headers: { Authorization: `Bearer ${token}` } 
          }).catch(e => ({ data: { count: 0 } })), // Fallback if error
          axios.get('http://69.62.83.14:5000/api/hod/student-today-attendance-count', { 
            headers: { Authorization: `Bearer ${token}` } 
          }).catch(e => ({ data: { count: 0 } })),
          axios.get('http://69.62.83.14:5000/api/hod/nonteaching-today-attendance-count', { 
            headers: { Authorization: `Bearer ${token}` } 
          }).catch(e => ({ data: { count: 0 } }))
        ]);
...
function RecentFacultyLogs({ facultyMembers = [], handlePersonClick }) {
  ...
  useEffect(() => {
    ...
    const fetchLogs = async () => {
      ...
      try {
        const token = localStorage.getItem('token');
        const res = await fetch('http://69.62.83.14:5000/api/hod/recent-faculty-logs', {
          headers: { 'Authorization': `Bearer ${token}` },
        });
...
function RecentLeaveApprovals() {
  ...
  useEffect(() => {
    const fetchLeaves = async () => {
      try {
        setLoading(true);
        const res = await axios.get('http://69.62.83.14:5000/api/hod/leave-approval');
  ...
  const handleAction = async (application, action) => {
    ...
    try {
      await axios.put(`http://69.62.83.14:5000/api/hod/leave-approval/${application.ErpStaffId}`, {
